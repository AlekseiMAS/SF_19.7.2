...
def test_get_api_key_valid_user(email=valid_email, password=valid_password):
    '''Проверка статуса код 200, в переменной result есть слово key'''
    status, result = pf.get_app_key(email, password)
    assert status == 200
    assert 'key' in result

def test_get_all_pets_with_valid_key(filter=''):
    '''Проверка статуса код 200, список питомцев не пустой.
    Получаем ключ - метод get_app_key() , сохраняем его в api_key,
    применяем метод get_list_of_pets(), проверка статуса ответа и списка питомцев'''
    _, api_key = pf.get_app_key(valid_email, valid_password)
    status, result = pf.get_list_of_pets(api_key, filter)
    assert status == 200
    assert len(result['pets']) > 0

def test_add_pets_with_valid_data(name='Petya', animal_type='cat', age='2', pet_photo='images/Petya.jpg'):
    '''Проверка статуса код 200, список с добавленными данными не пустой.
    В переменную pet_photo сохраняем путь к файлу фотографии питомца, сохраняем ключ в переменную api_key,
    проверяем статус ответа и содержание в ответе добавленных данных'''
...
def test_delete_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pets(api_key, 'Tuzik', 'cat', '5', 'images/Petya.jpg')
        _, my_pets = pf.get_list_of_pets(api_key, 'my_pets')
...
def test_add_pets_with_valid_data_without_photo(name='CatNoPhoto', animal_type='кот', age='6'):
...
def test_add_pet_negative_age_number(name='Petya', animal_type='cat', age='-2', pet_photo='images/Petya.jpg'):
...
def test_add_pet_with_four_digit_age_number(name='Petya', animal_type='cat', age='1234', pet_photo='images/Petya.jpg'):
...
def test_add_pet_with_empty_value_in_variable_name(name='', animal_type='cat', age='2', pet_photo='images/Petya.jpg'):
...
def test_add_pet_with_a_lot_of_words_in_variable_name(animal_type='cat', age='2', pet_photo='images/Petya.jpg'):
    ...
    name = 'dkgj dfgkjflgk  fdjgfldlldf dflldfgjgj dfgkfgkgkgkk fdglldfll fglldfgl ldg ldfgjshh dfgjfg jjdg  dfghdfjgh  dgjhdfgjh df gjjgjhdhgjdllj  dsjhfhsf sdfdfff ffffff'
...
def test_add_pet_with_special_characters_in_variable_animal_type(name='Petya', age='2', pet_photo='images/Petya.jpg'):
...
def test_add_pet_with_numbers_in_variable_animal_type(name='Petya', animal_type='34562', age='2',
                                                      pet_photo='images/Petya.jpg'):
...
def test_add_pet_with_a_lot_of_words_in_variable_animal_type(name='Petya', age='2', pet_photo='images/Petya.jpg'):
    ...
    animal_type = 'dklfgj erer dhfghfj sfsdfd jgjgfgfsdfd sfsdfsdf hfhfrdg asfsdfgdg afdfghfgh jkghjgh sgdfgsdf hfghfgh jgyjghh'
